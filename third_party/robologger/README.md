<!--
 Copyright (c) 2024 Anonymous Authors (name withheld for double-blind review)
...
`robologger` is a distributed data collection library for robot learning. Each modality (robot arm, camera, gripper, mobile base) runs its own logger process at its own frequency; a central `MainLogger` coordinates episode start/stop via `robotmq`. Numerical data is stored in zarr; video is encoded to MP4 via FFmpeg.

**Install:**
```bash
pip install -e third_party/robologger  # from the repository root
```
...
`robologger` provides a logger class for each modality, all controlled by a single `MainLogger` via `robotmq` message passing. Data is saved in a standardized zarr directory structure with episode-level metadata.
...
cd modpack/third_party/robologger
pip install -e .
```
